...
import { createInMemoryCableRepository } from "../inMemoryCableRepository";
import type {
  CableRecord,
...
function makeRepo() {
  return createInMemoryCableRepository();
}
...
const result = {
  designCurrentA: 40,
  maxVoltageDropV: 1.44,
  calculatedAreaMm2: 23.2,
  ampacityAreaMm2: 10,
  requiredAreaMm2: 23.2,
  selectedAreaMm2: 25,
  actualVoltageDropV: 1.34,
  actualDropPercent: 2.79,
  resistanceOhmPerKm: 0.86,
  temperatureFactor: 1,
  groupingFactor: 1,
  deratedAmpacityA: 101,
  warnings: [],
  errors: [],
  isValid: true,
};

const baseInput: CreateCableRecordInput = {
  projectId: "p1",
  auditId: "aud-1",
  label: "Battery main",
  circuitRole: "battery",
...
describe("list", () => {
  ...
  it("filters by circuitRole", async () => {
    const repo = makeRepo();
    await repo.create({ ...baseInput, circuitRole: "battery" });
    await repo.create({ ...baseInput, circuitRole: "pv-string" });

    const batteries = await repo.list({ circuitRole: "battery" });
    expect(batteries.length).toBe(1);
    expect(batteries[0]!.circuitRole).toBe("battery");
  });
...
describe("update", () => {
  ...
  it("does not allow overwriting id or createdAt", async () => {
    ...
    const updated = await repo.update(created.id, {
      // @ts-expect-error — id is not part of UpdateCableRecordPatch
      id: "hacked",
    });
...
describe("select", () => {
  ...
  it("archives a previously selected record for the SAME project AND role", async () => {
    const repo = makeRepo();
  ...
  it("does NOT archive a selected record for a DIFFERENT role", async () => {
    const repo = makeRepo();
    const battery = await repo.create({ ...baseInput, circuitRole: "battery" });
    const pvString = await repo.create({
      ...baseInput,
      circuitRole: "pv-string",
    });

    await repo.select(battery.id);
    await repo.select(pvString.id);

    const batteryAfter = await repo.get(battery.id);
    const pvAfter = await repo.get(pvString.id);
    expect(batteryAfter!.status).toBe("selected");
    expect(pvAfter!.status).toBe("selected");
  });
...
describe("seed", () => {
  it("accepts a seed array", async () => {
    ...
    const repo = createInMemoryCableRepository(seed);
    const list = await repo.list();
